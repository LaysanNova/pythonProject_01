...
class TestMakeItemPurchase:
    ...
    def test_check_prices_and_total_amount(self):
        ...
        price_on_home_page = page_item.sl.wait_until_element_is_visible(
            HomePage.BLOCK_NOKIA_LUMIA_1520
        ).text
        ...
        price_on_product_page = page_product.sl.wait_until_element_is_visible(
            ProductPage.PRICE_CONTAINER
        ).text
        price_on_product_page = int(
            price_on_product_page.split(" ")[0].replace("$", "")
        )
        assert (
            price_on_home_page == price_on_product_page
        ), "Price on product page is not placed."
        ...
        price_on_cart_page = page_cart.sl.wait_until_element_is_visible(
            CartPage.PRODUCTS_IN_CART
        ).text
        price_on_cart_page = page_cart.get_product_price(price_on_cart_page)

        assert (
            price_on_product_page == price_on_cart_page
        ), "Price in the cart is not placed."
        ...
        sum_of_products_price = sum(
            list(map(page_cart.get_product_price, product_list))
        )
        total = int(page_cart.sl.wait_until_element_is_visible(CartPage.TOTAL).text)
        assert (
            sum_of_products_price == total
        ), "Total price does not match to sum of the product's items."
